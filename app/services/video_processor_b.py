...
async def split_video(video_content: bytes, task_id: str) -> List[bytes]:
    ...
    temp_output_files = []  # Store temp output file paths
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as temp_file:
            ...
            temp_input_file = temp_file.name  # Save path before closing
        ...
        num_parts = min(5, max(1, int(duration // 60)))  # 1 part per minute, max 5
        frames_per_part = total_frames // num_parts
        frame_skip = max(1, fps // 2)  # Skip frames to reduce size
        ...
        video_parts = []
        progress_per_part = 7 / num_parts  # 7% progress per part

        for i in range(num_parts):
            task_tracker.update_progress(task_id, f"Processing video part {i + 1}/{num_parts}",
                                         8 + (i * progress_per_part))
            ...
            writer = cv2.VideoWriter(temp_output_files[-1], fourcc, fps, (width, height))
            ...
            while frame_count < end_frame:
                ...
                if not ret:
                    break
                if frame_count % frame_skip == 0:  # Reduce frame count to optimize size
                    writer.write(frame)
                frame_count += 1

            writer.release()  # Close writer before reading file

            # Read the chunk back into memory
            with open(temp_output_files[-1], 'rb') as f:
        ...
        for temp_output_file in temp_output_files:
            if os.path.exists(temp_output_file):
                try:
                    ...
                    logger.error(f"Error deleting temporary output file {temp_output_file}: {str(e)}")
